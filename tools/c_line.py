...
def cmd_demo(args: argparse.Namespace) -> int:
    ...
    log("Step 3/5: generating AI Act summary / gaps / pack...")
    rc = run_cmd([
        sys.executable,
        "compliance_ai_act.py",
        receipts,                             # positional input
        "--out-summary",
        f"docs/AI_ACT_summary_{period}.md",
        "--out-gaps",
        f"data/AI_ACT_gaps_{period}.csv",
        "--out-pack",
        f"data/AI_ACT_pack_{period}.json",
    ])
    if rc != 0:
        log(f"compliance_ai_act.py failed with exit code {rc}. Aborting demo.")
        return rc
