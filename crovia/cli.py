...
def cmd_scan(args: argparse.Namespace) -> None:
    ...
    print_warn("crovia scan is not yet available in the open core.")
    print()
    print_section("What crovia scan does")
    print("  crovia scan takes a raw dataset (NDJSON) and automatically produces")
    print("  royalty_receipt.v1 attribution records by matching your dataset content")
    print("  against a known-corpus index.")
    print()
    print("  This requires a vector similarity index (FAISS) pre-built from known")
    print("  training corpora (Common Crawl, The Pile, LAION, etc.).")
    print("  That index is large (~GB) and is not yet distributed with the open core.")
    print()
    print_section("Why it is not included yet")
    print("  Building and distributing the corpus index requires:")
    print("  - Licensing agreements for the reference corpora")
    print("  - A hosted index endpoint or downloadable snapshot")
    print("  - The shard sketch generator (shard_sketch_gen.py) — in development")
    print()
    print_section("What you can do now")
    print_suggestion("If you already have royalty_receipt.v1 files, use: crovia check <file>")
    print_suggestion("To run the full evidence pipeline: crovia run --receipts <file> --period YYYY-MM --budget N --out <dir>")
    print_suggestion("To check a model's disclosure gaps: crovia oracle scan <org/model>")
